...
let con = {}
a.forEach(l => {
    con[l[0]] = con[l[0]]?.concat(l[1]) || [l[1]];
    con[l[1]] = con[l[1]]?.concat(l[0]) || [l[0]];
});
let trips = 0
Object.keys(con).forEach(i => con[i].forEach(j => con[j].forEach(k => {
    if (con[k].includes(i) && [i[0], j[0], k[0]].includes('t')) trips++;
})));
log(trips/6)


function biggestclique(R, P, X) {
    if (P.size === 0 && X.size === 0) {
        return [Array.from(R)];
    }
    let cliques = [];
    for (let v of P) {
        let newR = new Set(R).add(v)
        let newP = new Set([...P].filter(u => con[v].includes(u)))
        let newX = new Set([...X].filter(u => con[v].includes(u)));
        cliques = cliques.concat(biggestclique(newR, newP, newX));
        P.delete(v);
        X.add(v);
    }
    return cliques;
...
console.log(biggestclique(new Set(), new Set(Object.keys(con)), new Set()).sort((a, b) => b.length - a.length)[0].sort().join(','))
