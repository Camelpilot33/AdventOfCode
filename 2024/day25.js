...
let locks = a.filter(e=>e[0]=="#####"&&e[6]==".....").map(e => e.map((_, i) => e.map(f => f[i]).join``.replace(/\.*/g, '').length - 1).slice(0, -2));
let keys = a.filter(e=>e[0]=="....."&&e[6]=="#####").map(e => e.map((_, i) => e.map(f => f[i]).join``).map(w => w.replace(/\.*/g, '').length - 1).slice(0, -2));

let s=0
for (let l of locks) {
    for (let k of keys) {
        if (l.map((e,i)=>(e+k[i])<6).reduce((a,b)=>a&&b)) s++
    }
}
log(s)