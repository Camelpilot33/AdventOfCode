...
console.log(
    input.match(/mul\(\d+,\d+\)/g).map(x=>x.match(/\d+/g)).map(x=>x[0]*x[1]).sum()
)


let match = input.match(/mul\(\d+,\d+\)|do\(\)|don't\(\)/g);
let s=0;
let enable = true
for (v of match) {
    if (v == "do()") enable = true;
    else if (v == "don't()") enable = false;
    else if (enable) s += Number(v.match(/\d+/g)[0]) * Number(v.match(/\d+/g)[1]);
}
